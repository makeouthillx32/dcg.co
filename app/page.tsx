...
export function RootShell({
  ...
  const { session: clientSession } = useAuth();

  // ✅ prefer live client session when available, fallback to server session
  ...
  useEffect(() => {
    ...
    const pageCategory =
      route.isHome ? "landing" : route.isToolsPage ? "tools" : route.isDashboardPage ? "dashboard" : "general";
    ...
    if (typeof requestIdleCallback !== "undefined") requestIdleCallback(scheduleTracking);
    else setTimeout(scheduleTracking, 0);
  }, [pathname, route]);
  ...
  const cookieVariant =
    screenSize === "mobile" ? "small" : screenSize === "tablet" ? "mini" : "default";
    ...
    <CartProvider>
      ...
      {useAppHeader ? (
        ...
        <>
          ...
          <ShopHeader session={session} onMenuClick={() => setMobileMenuOpen(true)} />
...
/**
 * ✅ Your actual home page content still lives here.
 * If your home page is elsewhere, import it and return it here.
 */
export default function Page() {
  return null; // <-- replace with your real landing content component
}