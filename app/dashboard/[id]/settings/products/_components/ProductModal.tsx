  ...
  const uploadAll = async () => {
    ...
    try {
      ...
      for (const file of files) {
        const object_path = `products/${productId}/${randId()}.jpg`;
      ...
      <DialogContent className="max-w-4xl p-0 overflow-hidden">
        <div className="border-b border-[hsl(var(--border))] bg-[hsl(var(--background))]">
          <DialogHeader className="px-5 py-4">
            <DialogTitle className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => onOpenChange(false)} className="text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] transition-colors" aria-label="Close">
                  <X size={18} />
                </button>
              </div>
            </DialogTitle>
          ...
          <div className="flex border-t border-[hsl(var(--border))]">
            <button className={`px-4 py-2 font-medium text-sm ${activeTab === "details" ? "text-[hsl(var(--sidebar-primary))] border-b-2 border-[hsl(var(--sidebar-primary))]" : "text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] transition-colors"}`} onClick={() => setActiveTab("details")} disabled={!productId}>
              <span className="inline-flex items-center gap-2"><Settings2 size={16} /> Details</span>
            </button>
            <button className={`px-4 py-2 font-medium text-sm ${activeTab === "media" ? "text-[hsl(var(--sidebar-primary))] border-b-2 border-[hsl(var(--sidebar-primary))]" : "text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] transition-colors"}`} onClick={() => setActiveTab("media")} disabled={!productId}>
              <span className="inline-flex items-center gap-2"><ImageIcon size={16} /> Photos</span>
            </button>
            <button className={`px-4 py-2 font-medium text-sm ${activeTab === "tags" ? "text-[hsl(var(--sidebar-primary))] border-b-2 border-[hsl(var(--sidebar-primary))]" : "text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] transition-colors"}`} onClick={() => setActiveTab("tags")} disabled={!productId}>
              <span className="inline-flex items-center gap-2"><TagIcon size={16} /> Tags</span>
            </button>
          </div>
        </div>
          ...
          {!productId ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button type="button" variant="secondary" onClick={onAutoSlug}>Auto</Button>
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-semibold">Price (USD)</label>
                <Input value={formPrice} onChange={(e) => setFormPrice(e.target.value)} />
                <p className="text-xs text-[hsl(var(--muted-foreground))]">Current: {centsToMoney(detail.price_cents, detail.currency)}</p>
              </div>
              <div className="space-y-2">
              ...
              <div className="md:col-span-2 space-y-2">
                ...
                <Textarea value={formDesc} onChange={(e) => setFormDesc(e.target.value)} className="min-h-[120px]" />
              </div>
              <div className="flex items-center gap-3">
                <input id="featured" type="checkbox" checked={formFeatured} onChange={(e) => setFormFeatured(e.target.checked)} />
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 flex-wrap">
                  {(["draft", "active", "archived"] as const).map((s) => (
                    <button key={s} type="button" className={`px-3 py-1 rounded-full border text-sm ${formStatus === s ? "border-[hsl(var(--sidebar-primary))] text-[hsl(var(--sidebar-primary))]" : "border-[hsl(var(--border))] text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]"}`} onClick={() => setFormStatus(s)}>{s}</button>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4">
                ...
                <p className="text-xs text-[hsl(var(--muted-foreground))] mt-1">Multi-upload supported. Bucket: <code>{PRODUCT_IMAGE_BUCKET}</code></p>
                <div className="mt-3 space-y-2">
                  ...
                  <Button onClick={uploadAll} disabled={!files.length || uploading}>{uploading ? "Uploading…" : `Upload ${files.length ? `(${files.length})` : ""}`}</Button>
                </div>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {(detail.product_images ?? []).slice().sort((a, b) => (a.position ?? 0) - (b.position ?? 0)).map((img, idx) => {
                  const url = supabasePublicUrlFromImage(img);
                  return (
                    <div key={`${img.bucket_name ?? "bucket"}:${img.object_path ?? "path"}:${idx}`} className="rounded-lg border border-[hsl(var(--border))] overflow-hidden bg-[hsl(var(--background))]">
                      {url ? (
                        <img src={url} alt={img.alt_text ?? "product image"} className="w-full aspect-square object-cover" loading="lazy" />
                      ) : (
                        <div className="w-full aspect-square flex items-center justify-center text-xs text-[hsl(var(--muted-foreground))]">Missing URL</div>
                      )}
                    </div>
                  );
                })}
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4">
                ...
                <p className="text-xs text-[hsl(var(--muted-foreground))] mt-1">Use tags as subcategories (ex: <code>tops</code>, <code>denim</code>, <code>accessories</code>).</p>
                <div className="mt-3 flex gap-2">
                  <Input value={tagInput} onChange={(e) => setTagInput(e.target.value)} placeholder="Add tag… (ex: Date Night)" onKeyDown={(e) => { if (e.key === "Enter") addTag(); }} />
              ...
              <div className="flex flex-wrap gap-2">
                {(detail.tags ?? []).length === 0 ? (
                  ...
                  (detail.tags ?? []).map((t) => (
                    <span key={t.id} className="inline-flex items-center gap-2 rounded-full border border-[hsl(var(--border))] px-3 py-1 bg-[hsl(var(--background))]">
                      <span className="text-sm font-semibold">{t.name}</span>
                      <button className="text-xs text-[hsl(var(--muted-foreground))] hover:opacity-80" onClick={() => removeTag(t.id)} title="Remove">✕</button>
                    </span>
                  ))
                )}
              </div>
              <div className="text-xs text-[hsl(var(--muted-foreground))]">Tip: keep tags consistent. Enter "Date Night" once, then reuse it everywhere.</div>
            </div>
          )}
        </div>
        <div className="flex items-center justify-between gap-2 border-t border-[hsl(var(--border))] bg-[hsl(var(--muted))] p-4">
          <Button variant="secondary" onClick={load} disabled={loading || !productId}>Refresh</Button>
          {activeTab === "details" ? (
            <div className="flex gap-2">
