...
import { useTheme, useAuth } from "./provider";
...
function useScreenSize() {
  const [screenSize, setScreenSize] = useState<"mobile" | "tablet" | "desktop">(() => {
    ...
    try {
      ...
      try { setCookie("screenSize", size, { maxAge: 86400 }); } catch (e) {}
      return size;
    } catch (e) {
      return "desktop";
    }
      ...
      timeoutId = setTimeout(() => {
        try {
          ...
          if (newSize !== screenSize) {
            ...
            try { setCookie("screenSize", newSize, { maxAge: 86400 }); } catch (e) {}
          }
        } catch (e) {}
      }, 200);
    };
...
function classifyRoute(pathname: string) {
  ...
  return {
    ...
    isCategoryPage: /^\/[^\/]+$/.test(pathname) && !lower.startsWith("/tools") && !lower.startsWith("/dashboard") && !lower.startsWith("/products") && !lower.startsWith("/auth"),
  };
}
...
function RootLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { setMobileMenuOpen(false); }, [session?.user?.id]);
  useEffect(() => {
      ...
      <SidebarProvider>
        ...
        <div className="flex min-h-screen">
          ...
          <div className="w-full">
            <DashboardHeader />
            <main className="isolate mx-auto w-full max-w-screen-2xl overflow-hidden p-4 md:p-6 2xl:p-10">{children}</main>
          </div>
        </div>
      </SidebarProvider>
    );
  ...
  if (route.isAuthPage) {
    return (
      <>
        <RegionBootstrap />
        {children}
        <Toaster position="top-right" toastOptions={{ duration: 3000, style: { background: "hsl(var(--background))", color: "hsl(var(--foreground))", border: "1px solid hsl(var(--border))" } }} />
      </>
    );
  }
  ...
  return (
    <CartProvider>
      <RegionBootstrap />
      {useAppHeader ? <AppHeader /> : isShopRoute ? (
        <>
          <ShopHeader onMenuClick={() => setMobileMenuOpen(true)} />
          {mobileMenuOpen && (
            <>
              <div className="fixed inset-0 z-40 bg-black/50 transition-opacity duration-300 lg:hidden" onClick={() => setMobileMenuOpen(false)} aria-hidden="true" />
              <div className="fixed bottom-0 left-0 top-0 z-50 w-[min(86vw,360px)] overflow-y-auto border-r border-[var(--lt-border)] bg-[var(--lt-bg)] shadow-[var(--lt-shadow)] lg:hidden" data-layout="shop">
                <MobileDrawer key={session?.user?.id || 'guest'} onClose={() => setMobileMenuOpen(false)} />
              </div>
            </>
          )}
        </>
      ...
      <Suspense fallback={<div style={{ minHeight: '1px' }} />}>
        ...
        <CookieConsent variant={cookieVariant} showCustomize={screenSize !== "mobile"} description={screenSize === "mobile" ? "We use cookies to enhance your experience. Essential cookies are required for functionality." : screenSize === "tablet" ? "We use cookies to enhance your experience and analyze usage. Essential cookies required." : "We use cookies to enhance your experience, analyze site usage, and improve our services. Essential cookies are required for basic functionality."} />
      </Suspense>
      <ConditionalOverlays />
      <Toaster position="top-right" toastOptions={{ duration: 3000, style: { background: "hsl(var(--background))", color: "hsl(var(--foreground))", border: "1px solid hsl(var(--border))" }, success: { iconTheme: { primary: "hsl(var(--primary))", secondary: "hsl(var(--primary-foreground))" } }, error: { iconTheme: { primary: "hsl(var(--destructive))", secondary: "hsl(var(--destructive-foreground))" } } }} />
    </CartProvider>
  );
}
  ...
  return (
    <html lang="en" suppressHydrationWarning>
      <head>
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "Brand", name: "Desert Cowgirl", description: "Western-inspired pants and shirts with a warm, modern rustic aesthetic.", url: "https://desertcowgirl.co/", logo: "https://desertcowgirl.co/logo.png", sameAs: [] }) }} />
      </head>
